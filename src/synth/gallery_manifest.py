...
GalleryArtifactSource = tuple[Path, GalleryArtifactKind, str]
...
def repository_relative_output_path(repo_root: Path, path: Path) -> Path:
    resolved_root = repo_root.resolve(strict=True)
    resolved_path = path.resolve(strict=False)
    relative = resolved_path.relative_to(resolved_root)
    if relative.parts[:2] != ("data", "outputs"):
        raise ValueError(f"Expected an output path, received {resolved_path}")
    return relative


def publish_gallery_run(
    repo_root: Path,
    files_directory: Path,
    generator: str,
    page_path: str,
    sources: tuple[GalleryArtifactSource, ...],
) -> Path:
    artifacts = tuple(
        GalleryArtifact.from_file(repo_root, path, kind, role)
        for path, kind, role in sources
    )
    return write_gallery_manifest(
        repo_root=repo_root,
